...
#
# Runs every hermetic validation step, including the *strict* clippy pass over all
# targets (`just clippy-all`) which the lightweight `check-clippy` CI gate does not
# cover, plus the hermetic test suite. Run it from the repo root:
#
